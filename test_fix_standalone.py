"""Standalone test script to verify the fix for option_pricer_half_time IndexError.
This version doesn't require the full dependencies, just numpy."""
import numpy as np
import sys
...
def option_pricer_half_time(paths, strike, r, T, option_type):
    ...
    This is a copy of the fixed function from mmar.py
    """
...
def option_pricer(paths, strike, r, T, option_type):
    ...
    This is a copy of the working function from mmar.py for comparison.
    """
    if isinstance(paths, list):
        paths = np.array(paths)
